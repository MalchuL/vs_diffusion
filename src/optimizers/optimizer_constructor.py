...
class DefaultOptimizerConstructor:
    ...
    def __init__(self,
                 optimizer_cfg: [Dict, omegaconf.dictconfig.DictConfig],
                 paramwise_cfg: Optional[omegaconf.dictconfig.DictConfig] = None):
        if isinstance(optimizer_cfg, omegaconf.dictconfig.DictConfig):
            optimizer_cfg = OmegaConf.to_container(optimizer_cfg, resolve=True)

        if isinstance(paramwise_cfg, omegaconf.dictconfig.DictConfig):
            paramwise_cfg = OmegaConf.to_container(paramwise_cfg, resolve=True)
    ...
    def add_params(self,
        ...
        is_norm = isinstance(module, (nn.BatchNorm2d, nn.InstanceNorm2d, GroupNorm, LayerNorm))
        is_dwconv = (
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            for key in sorted_keys:
                if key in f'{prefix}.{name}':
                    ...
                    logging.info(f'Special lr weights for {prefix}.{name} - {lr_mult}')
    ...
    def __call__(self, model):
        ...
        if not self.paramwise_cfg:
            ...
            return instantiate(optimizer_cfg, _convert_=True)

        # set param-wise lr and weight decay recursively
        params = []
        self.add_params(params, model)
        optimizer_cfg['params'] = params
        return instantiate(optimizer_cfg, _convert_='all')
